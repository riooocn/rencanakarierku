...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:hidden">
    <a href="{{ route('superadmin.peserta.show', $peserta->id) }}" class="text-primary-600 font-medium hover:underline">&larr; Kembali ke Riwayat Peserta</a>
    
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
        <span class="px-3 py-2 sm:py-1 bg-primary-100 text-primary-800 text-xs font-bold rounded-lg border border-primary-200 text-center">Data Terpusat</span>
    ...
    <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 mb-10 border-b border-slate-100 pb-8">
        ...
        <div class="flex-1 w-full">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-700 mt-4 text-left">
                ...
                <div class="flex items-start"><span class="font-semibold w-24 shrink-0">Instansi:</span> <span>{{ $peserta->institution->name ?? 'Instansi' }}</span></div>
                <div class="flex items-start"><span class="font-semibold w-24 shrink-0">Status Tes:</span> <span>{!! $keputusan->id !== 'incomplete' ? '<span class="text-green-600 font-medium">Selesai</span>' : '<span class="text-yellow-600 font-medium">Belum Selesai</span>' !!}</span></div>
            </div>
        </div>
    </div>
