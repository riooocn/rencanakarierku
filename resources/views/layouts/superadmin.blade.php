...
<body class="antialiased font-sans text-neutral-dark bg-neutral-light selection:bg-primary-500 selection:text-white flex h-screen overflow-hidden print:overflow-visible print:h-auto">
    ...
    <!-- Sidebar -->
    <aside class="w-64 bg-primary-900 border-r border-primary-800 flex flex-col hidden md:flex shrink-0 text-white print:hidden">
        <!-- Logo -->
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
            <div class="text-xs font-semibold text-primary-300 uppercase tracking-wider mb-2 mt-4 px-3">Super Admin</div>
            
            <a href="{{ route('superadmin.dashboard') }}" class="{{ request()->routeIs('superadmin.*') ? 'bg-accent-600 text-white' : 'text-primary-200 hover:bg-primary-800 hover:text-white' }} group flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-colors mb-1">
                <svg class="{{ request()->routeIs('superadmin.*') ? 'text-white' : 'text-primary-400 group-hover:text-primary-200' }} mr-3 flex-shrink-0 h-5 w-5 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="p-4 border-t border-primary-800 bg-primary-950">
            <div class="flex items-center w-full px-3 py-2">
                ...
                <div class="flex-1 truncate">
                    ...
                    <p class="text-xs text-primary-300 truncate">super@example.com</p>
                </div>
                <button class="text-primary-300 hover:text-red-400 transition-colors" title="Logout">
    ...
    <div class="flex-1 flex flex-col overflow-hidden print:overflow-visible print:h-auto">
        ...
        <header class="md:hidden h-16 bg-primary-900 border-b border-primary-800 flex items-center justify-between px-4 print:hidden">
            <a href="{{ Auth::user()->role === 'superadmin' ? route('superadmin.dashboard') : route('admin.dashboard') }}" class="inline-flex items-center gap-2">
                <img src="{{ asset('images/logo.png') }}" alt="Logo" class="w-8 h-8 object-contain">
            </a>
            <button class="text-primary-300 hover:text-white focus:outline-none">
