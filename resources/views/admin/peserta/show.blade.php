        ...
        <div class="space-y-4">
            @foreach($riwayatList as $index => $riwayat)
                <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md hover:border-primary-200 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div>
                        <a href="{{ route('admin.peserta.history.show', ['id' => $peserta->id, 'history_id' => $riwayat->id]) }}" class="inline-flex items-center justify-center px-4 py-2 bg-slate-100 text-slate-700 hover:bg-primary-50 hover:text-primary-700 font-medium rounded-xl transition-colors w-full sm:w-auto">
                            Lihat Detail
